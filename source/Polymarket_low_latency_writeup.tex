...
\begin{titlepage}
    ...
    \begin{center}
        {\LARGE\bfseries Low-Latency Cross-Venue Arbitrage:\\[0.4em]
         Binance $\rightarrow$ Polymarket} \\[1.2em]
        {\large Strategy, Infrastructure, Deployment, and a Production Post-Mortem} \\[2em]
...
protocol, lock-free per-asset processing pipelines, a full mock-to-live execution
...
resting size, roughly one order in ten filled. The edge was priced into a book that
disappeared as soon as a real order tried to cross it.
...
US) imposes an $\sim$120\,ms one-way transit paid on every signal. The system
...
majority of the strategy's profit came from the 5-minute windows, where the higher
...
shown on the book was gone by the time the order arrived.
...
built to do, on time. The strategy still lost, because a latency arbitrage implicitly
assumes that winning the race entitles you to the liquidity at the finish line, and
in this venue it did not.
...
would have been best. A displayed-book backtest cannot represent this, because it
...
applying a haircut calibrated to observed live fill rates) would have turned the
$61\%$-win-rate backtest into a losing one and prevented the deployment. The point
...
lock-free per-asset pipelines, the mock-to-live parity, and the calibration and
...
cheap, stable, and correct. The strategy still lost money, because the liquidity it
was designed to capture was largely illusory, and because the paper harness, faithful
to the displayed book, could not detect this until real capital met the real book.
...
lock-free processing model, and a mock-to-live harness with faithful API parity. The
